        ...
        if ($widgetKey === 'search') { ?>
            ...
            <div class="widget-card">
                ...
                <form action="{{ pathto('') }}" method="GET" class="input-group">
                    ...
                    <button class="btn btn-sm btn-info text-white" type="submit"><i class="fa-solid fa-search"></i></button>
                </form>
            </div>
                    ...
                    <?php } else { ?>
                        <?php foreach ($widget_categories as $cat) { ?>
                            <li class="mb-2 d-flex justify-content-between align-items-center">
                                <a href="{{ pathto('category/' . $cat['slug']) }}" class="text-decoration-none small text-dark"><i class="fa-solid fa-anchor me-2 text-info"></i> {{ htmlspecialchars($cat['name'], ENT_QUOTES, 'UTF-8') }}</a>
                            </li>
                        <?php } ?>
                    <?php } ?>
                </ul>
                ...
                <ul class="list-unstyled mb-0">
                    ...
                    <?php } else { ?>
                        <?php foreach ($widget_pages as $page) { ?>
                            <li class="mb-2">
                                <a href="{{ pathto('page/' . $page['slug']) }}" class="text-decoration-none small text-dark"><i class="fa-solid fa-compass me-2 text-success"></i> {{ htmlspecialchars($page['title'], ENT_QUOTES, 'UTF-8') }}</a>
                            </li>
                        <?php } ?>
                    <?php } ?>
                </ul>
