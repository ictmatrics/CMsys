    ...
    <div class="row">
        ...
        <div class="{{ $mainCols }}">
            <article class="bg-white p-4 p-md-5 rounded-4 shadow-sm border border-light mb-5">
                <header class="mb-4">
                    ...
                    <div class="text-muted small my-3 d-flex flex-wrap align-items-center gap-3">
                        <span><i class="fa-solid fa-calendar-days me-2"></i> {{ date('M d, Y', strtotime($post->created_at)) }}</span>
                        <?php if (!empty($post_categories)) { ?>
                            <span>
                                <i class="fa-solid fa-anchor me-2 text-info"></i>
            ...
            <section class="bg-white p-4 p-md-5 rounded-4 shadow-sm border border-light mb-5">
                <h4 class="font-weight-bold mb-4"><i class="fa-solid fa-comments me-2 text-info"></i> Comments ({{ count($comments) }})</h4>
                ...
                <div class="comments-list mb-5">
                    ...
                    <?php } else { ?>
                        <?php foreach ($comments as $comment) { ?>
                            <div class="d-flex mb-4 p-3 bg-light rounded border-start border-info border-3">
                                <div class="flex-shrink-0 me-3">
                                    <div class="bg-info text-white rounded-circle d-flex align-items-center justify-content-center" style="width: 48px; height: 48px; font-weight:700;">
                ...
                <form action="{{ pathto('comment/add') }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-info text-white px-4 font-weight-bold"><i class="fa-solid fa-paper-plane me-2"></i> Submit Comment</button>
                </form>
            </section>
        </div>
